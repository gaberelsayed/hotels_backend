...
exports.reservationSearch = async (req, res) => {
	try {
		...
		console.log(
			searchQuery,
			"searchQuerysearchQuerysearchQuerysearchQuerysearchQuery"
		);
		...
		const isObjectIdValid = ObjectId.isValid(searchQuery);
		...
		// Add ObjectId queries if searchQuery is a valid ObjectId
		if (isObjectIdValid) {
			query.$or.push({ _id: new ObjectId(searchQuery) });
			query.$or.push({ belongsTo: new ObjectId(searchQuery) });
		}
...
exports.singleReservation = async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ error: "Error fetching reservation from the database" });
	}
};
...
exports.bookingDataDump = async (req, res) => {
	try {
		...
		const newRecords = data.filter((item) => {
			if (!item["Book Number"]) {
				console.log('Missing "Book Number" in item:', item);
				return false;
			}
		...
		newRecords.forEach((item) => {
			if (!item["Book Number"]) {
				// Skip this item because it doesn't have a "Book Number"
				return;
			}
			const itemNumber = item["Book Number"].toString().trim();
			// ... rest of your forEach logic
		});
